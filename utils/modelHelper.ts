...
import { cifar10Classes } from '../data/cifar10';

export async function runOnnxModel(preprocessedData: any): Promise<[any, number]> {
  try {
    const session = await ort.InferenceSession
      .create('/model/secondmodel_20250204223846.onnx',                  // Change deployed model here
...
export function imagenetClassesTopK(classProbabilities: any, k = 5) {
  ...
  const topK = _.take(sorted, k).map((probIndex: Array<number>) => {
    const iClass = cifar10Classes[probIndex[1]];
    return {
      id: iClass[0],
